...
from utils import print_program, rewrite_program_postgres, str_to_program, quivr_str_to_postgres_program, postgres_execute, str_to_program_postgres
...
def test_synthetic_queries(method, dataset_name, n_init_pos, n_init_neg, npred, depth, max_duration, nvars, beam_width, pool_size, k, samples_per_iter, budget, multithread):
    dir_name = "outputs/{}/{}/nip_{}-nin_{}-npred_{}-depth_{}-max_d_{}-nvars_{}-bw_{}-pool_size_{}-k_{}-per_iter_{}-budget_{}-thread_{}".format(method, dataset_name, n_init_pos, n_init_neg, npred, depth, max_duration, nvars, beam_width, pool_size, k, samples_per_iter, budget, multithread)
...
def _helper(args, dir_name, dataset_name, k):
    ...
    for i, (query_str, _) in enumerate(answers, start=1):
        postgres_program = str_to_program_postgres(target_query)
    ...
    if method.startswith("vocal_postgres"):
        if "scene_graph" in dataset_name:
            f1_score = get_f1_score_postgres_scene_graph(answers[:10], target_query, dataset_name)
        else:
...
def get_f1_score_postgres_scene_graph(test_queries, target_query, dataset_name):
    print("target_query", target_query)
    _start = time()
    with open("inputs/{}/test/{}_inputs.json".format(dataset_name, target_query), 'r') as f:
        inputs = json.load(f)
    ...
    for test_query, train_score in test_queries:
        ...
        result, new_memoize = postgres_execute(dsn, program, memoize, inputs_table_name, inputs.tolist(), is_trajectory=False)
        for input in inputs:
            if input in result:
                y_pred_per_query.append(1)
            else:
...
if __name__ == '__main__':
    ...
    ap.add_argument('--pool_size', type=int)
    ...
    with open("outputs/{}/{}/eval-nip_{}-nin_{}-npred_{}-depth_{}-max_d_{}-nvars_{}-bw_{}-pool_size_{}-k_{}-per_iter_{}-budget_{}-thread_{}.txt".format(method_str, dataset_name, n_init_pos, n_init_neg, npred, depth, max_duration, nvars, beam_width, pool_size, k, samples_per_iter, budget, multithread), 'w') as f:
        sys.stdout = f
        _start = time()
        test_synthetic_queries(method_str, dataset_name, n_init_pos, n_init_neg, npred, depth, max_duration, nvars, beam_width, pool_size, k, samples_per_iter, budget, multithread)
        print("Time: {}".format(time() - _start))