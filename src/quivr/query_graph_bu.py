...
from utils_bu import print_program
...
class QueryGraph(object):
    ...
    def get_all_children_bu(self):
        ...
        Verbose form:
            q = Seq(Seq(Seq(Seq(True*, Conj(Kleene(Conj(Conj(p13, p12), p11)), MinLength(theta1))), True*), Conj(Kleene(Conj(Conj(p23, p22), p21)), MinLength(theta2))), True*)
        """
        all_children = []
        ...
        if self.num_atomic_predicates + 1 <= self.max_num_atomic_predicates:
            for pred in predicate_dict:
                for param in predicate_dict[pred]:
                    new_query_graph = copy.deepcopy(self)
                    # 1. Find the last scene graph g2 = q.submodules["function1"].submodules["function2"] // g2 = Conj(Kleene(Conj(Conj(p23, p22), p21)), MinLength(theta2))
                    ...
                    # 2. If g2 has duration constraint, locate the scene graph only: n2 = g2.submodules["function1"].submodules["kleene"] // n2 = Conj(Conj(p23, p22), p21)
                    if isinstance(last_graph, dsl.ConjunctionOperator) and isinstance(last_graph.submodules["function2"], dsl.MinLength):
                        parent_graph = [last_graph.submodules["function1"], "kleene"]
                        last_graph = last_graph.submodules["function1"].submodules["kleene"]
                    ...
                    while isinstance(last_graph, dsl.ConjunctionOperator):
                        if last_graph.submodules["function2"] >= pred(param):
                    ...
                    if is_duplicate_predicate:
                        continue
                    if last_graph >= pred(param):
                        continue
                    # 4. Replace p23 with Conj(p24, p23)
                    parent_graph[0].submodules[parent_graph[1]] = dsl.ConjunctionOperator(pred(param), last_graph)
                    new_query_graph.num_atomic_predicates += 1
        ...
        if self.num_atomic_predicates + 1 <= self.max_num_atomic_predicates and self.depth + 1 <= self.max_depth:
            for pred in predicate_dict:
                for param in predicate_dict[pred]:
                    new_query_graph = copy.deepcopy(self)
                    new_query_graph.program = dsl.SequencingOperator(dsl.SequencingOperator(new_query_graph.program, pred(param)), dsl.TrueStar())
                    new_query_graph.num_atomic_predicates += 1
        ...
        # 1. Find the last scene graph g2 = q.submodules["function1"].submodules["function2"] // g2 = Conj(Kleene(Conj(Conj(p23, p22), p21)), MinLength(theta2))
        last_graph = new_query_graph.program.submodules["function1"].submodules["function2"]
        # 2. If g2 has duration constraint, increment by 1: g2.submodules["function2"].theta += 1
        # NOTE: maximum duration constraint is 10
        if isinstance(last_graph, dsl.ConjunctionOperator) and isinstance(last_graph.submodules["function2"], dsl.MinLength):
            if last_graph.submodules["function2"].theta < 10:
                last_graph.submodules["function2"].theta += 1
                all_children.append(new_query_graph)
        # 3. Else, add a duration constraint: g2' = Conj(Kleene(g2), MinLength(2))
        else:
            new_query_graph.program.submodules["function1"].submodules["function2"] = dsl.ConjunctionOperator(dsl.KleeneOperator(last_graph), dsl.MinLength(2))
