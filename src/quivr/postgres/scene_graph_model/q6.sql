/*
Duration(Conjunction(Brown(o0), Far_0.9(o1, o2)), 2); Duration(TopQuadrant(o1), 3); Conjunction(Conjunction(Conjunction(BottomQuadrant(o0), Red(o1)), RightOf(o0, o1)), TopQuadrant(o1))
*/
...
EXPLAIN ANALYZE CREATE TEMPORARY TABLE g0 AS SELECT o0.vid as vid, o0.fid as fid, o0.oid as o0_oid, o1.oid as o1_oid, o2.oid as o2_oid FROM Obj_filtered as o0, Obj_filtered as o2, Obj_filtered as o1 WHERE o0.vid = o1.vid and o0.fid = o1.fid and o1.vid = o2.vid and o1.fid = o2.fid and Brown(o0.shape, o0.color, o0.material, o0.x1, o0.y1, o0.x2, o0.y2) = true and Far(0.9, o1.shape, o1.color, o1.material, o1.x1, o1.y1, o1.x2, o1.y2, o2.shape, o2.color, o2.material, o2.x1, o2.y1, o2.x2, o2.y2) = true and o0.oid <> o1.oid and o0.oid <> o2.oid and o1.oid <> o2.oid;
...
EXPLAIN ANALYZE CREATE TEMPORARY TABLE g1 AS SELECT o1.vid as vid, o1.fid as fid, o1.oid as o1_oid FROM Obj_filtered as o1 WHERE TopQuadrant(o1.shape, o1.color, o1.material, o1.x1, o1.y1, o1.x2, o1.y2) = true;
CREATE INDEX IF NOT EXISTS idx_g1 ON g1 (vid, fid, o1_oid);

                    EXPLAIN ANALYZE CREATE TEMPORARY TABLE g1_seq_view AS
                    WITH RECURSIVE g1_seq (vid, fid1, fid2, o1_oid) AS (
                        SELECT vid, fid, fid, o1_oid FROM g1
                            UNION
                        SELECT s.vid, s.fid1, g.fid, s.o1_oid
                        FROM g1_seq s, g1 g
                        WHERE s.vid = g.vid and g.fid = s.fid2 + 1 and s.o1_oid = g.o1_oid and g.fid - s.fid1 + 1 <= 3
                    )
                    SELECT DISTINCT vid, fid1, fid2, o1_oid FROM g1_seq WHERE fid2 - fid1 + 1 = 3;

CREATE INDEX IF NOT EXISTS idx_g1_seq_view ON g1_seq_view (vid, fid1, fid2, o1_oid);
EXPLAIN ANALYZE CREATE TEMPORARY TABLE g2 AS SELECT o0.vid as vid, o0.fid as fid, o0.oid as o0_oid, o1.oid as o1_oid FROM Obj_filtered as o0, Obj_filtered as o1 WHERE o0.vid = o1.vid and o0.fid = o1.fid and BottomQuadrant(o0.shape, o0.color, o0.material, o0.x1, o0.y1, o0.x2, o0.y2) = true and Red(o1.shape, o1.color, o1.material, o1.x1, o1.y1, o1.x2, o1.y2) = true and RightOf(o0.shape, o0.color, o0.material, o0.x1, o0.y1, o0.x2, o0.y2, o1.shape, o1.color, o1.material, o1.x1, o1.y1, o1.x2, o1.y2) = true and TopQuadrant(o1.shape, o1.color, o1.material, o1.x1, o1.y1, o1.x2, o1.y2) = true and o0.oid <> o1.oid;
CREATE INDEX IF NOT EXISTS idx_g2 ON g2 (vid, fid, o0_oid, o1_oid);

                    EXPLAIN ANALYZE CREATE TEMPORARY TABLE g2_seq_view AS
                    WITH RECURSIVE g2_seq (vid, fid1, fid2, o0_oid, o1_oid) AS (
                        SELECT vid, fid, fid, o0_oid, o1_oid FROM g2
                            UNION
                    ...
                    SELECT DISTINCT vid, fid1, fid2, o0_oid, o1_oid FROM g2_seq WHERE fid2 - fid1 + 1 = 1;

CREATE INDEX IF NOT EXISTS idx_g2_seq_view ON g2_seq_view (vid, fid1, fid2, o0_oid, o1_oid);
                ...
                WHERE t0.vid = t1.vid and t0.fid2 < t1.fid1 and t0.o1_oid = t1.o1_oid
...
-- Results
-- 16
-- [116, 34, 205, 105, 6, 31, 287, 261, 117, 41, 206, 187, 191, 154, 177, 106]
-- time 5.917924880981445
