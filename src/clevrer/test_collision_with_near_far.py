...
def detect_collision(method_name, method, num_train, duration, objects_near, objects_far):
...
def _detect_collision(video_basename, maskrcnn_bboxes_evaluation, method, duration, objects_near, objects_far):
    ...
    for obj1_str_id, obj2_str_id in itertools.combinations(obj_set, 2):
        ...
        for chunk_id in range(4):
            ...
            if (method == "using_far_near_far" and collision_matching_by_far_near_far(near_fid_list, far_fid_list)) or (method == "using_far_near" and collision_matching_by_far_near(near_fid_list, far_fid_list)) or (method == "using_near" and collision_matching_by_near(near_fid_list, far_fid_list)) or (method == "using_far_near_or_near_far" and collision_matching_by_far_near_or_near_far(near_fid_list, far_fid_list)):
...
def compute_f1_score(method_name, misclassified_report):
    ...
    with open("/gscratch/balazinska/enhaoz/complex_event_video/src/clevrer/f1_scores-32_frames.txt", 'a') as f:
        f.write("method: {}; tp: {}; fp: {}; fn: {}; tn: {}; f1_score: {}\n".format(method_name, tp, fp, fn, tn, 2*tp/(2*tp+fp+fn)))
...
def _compute_f1_score(args, misclassified_report, collision_list, misclassified_filename):
    ...
    for collision in collisions:
        if collision["frame"] >= chunk_id * 32 and collision["frame"] < (chunk_id + 1) * 32:
...
if __name__ == '__main__':
    ...
    objects_far = joblib.load("/gscratch/balazinska/enhaoz/complex_event_video/src/outputs/clevrer_far/models/random_forest-{}-least_confidence-original.joblib".format(args.num_train))
    objects_near = joblib.load("/gscratch/balazinska/enhaoz/complex_event_video/src/outputs/clevrer_near/models/random_forest-{}-least_confidence-original.joblib".format(args.num_train))
    detect_collision(method_name, args.method, args.num_train, args.duration, objects_near, objects_far)
    compute_f1_score(method_name, args.misclassified_report)