...
def construct_input_stream_car_turning(connection, video_fn, pos_or_neg, n_samples=None):
    ...
    # car_stream: (frame_id, start_time, end_time)
    car_stream = []
    cursor = connection.cursor()
    cursor.execute("""
        SELECT v.frame_id, e.start_time, e.end_time,
            JSON_ARRAYAGG(v.x1), JSON_ARRAYAGG(v.x2),
            JSON_ARRAYAGG(v.y1), JSON_ARRAYAGG(v.y2)
        FROM Car c, Event e, VisibleAt v
        WHERE e.event_id = c.event_id
            AND e.event_id = v.event_id
            AND v.filename = %s
        GROUP BY v.frame_id, e.start_time, e.end_time
        ORDER BY v.frame_id
        """, [video_fn])
    results = cursor.fetchall()
    for row in results:
        x1_list, x2_list, y1_list, y2_list = [json.loads(lst) for lst in row[3:]]
        if has_car_at_intersection(intersection, x1_list, x2_list, y1_list, y2_list) is (pos_or_neg == "pos"):
            car_stream.append((row[:3]))
    if n_samples is not None:
        car_stream = sample(car_stream, n_samples)
