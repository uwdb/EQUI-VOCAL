...
CREATE TEMPORARY TABLE g1_windowed AS (
    SELECT vid, fid, o0_oid, o2_oid,
    lead(fid, 1 - 1, 0) OVER (PARTITION BY vid, o0_oid, o2_oid ORDER BY fid) as fid_offset
    FROM g1
);


CREATE TEMPORARY TABLE g1_contiguous AS (
    SELECT vid, o0_oid, o2_oid, fid AS fid1, fid_offset AS fid2
    FROM g1_windowed
...
CREATE TEMPORARY TABLE g2_windowed AS (
    SELECT vid, fid, o0_oid, o2_oid,
    lead(fid, 5 - 1, 0) OVER (PARTITION BY vid, o0_oid, o2_oid ORDER BY fid) as fid_offset
    FROM g2
);
...
SELECT DISTINCT t0.vid as vid, t1.fid2 as fid, t0.o0_oid as o0_oid, t0.o1_oid as o1_oid, t1.o2_oid as o2_oid
FROM q0 as t0, g1_contiguous as t1
WHERE t0.vid = t1.vid and t0.fid < t1.fid1 and t0.o0_oid = t1.o0_oid and t0.o0_oid <> t1.o2_oid and t0.o1_oid <> t1.o2_oid;
